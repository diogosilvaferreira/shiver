...
public class ProxySetCommand implements CommandMarker
{
	 ...
	 * @return
	 */
	@CliCommand(value = "proxy set", help = "Set a default proxy")
			...
			@CliOption(key = {"id"}, mandatory = true, help = "The proxy id") final int id)
	{
		...
		int rowCount = controller.updateDefaultProxy(id);
		if (rowCount > 0)
		{
			return PrettyPrint.print("Proxy successfuly updated!");
		}
					
		logger.severe(PrettyPrint.print("Unable to update the proxy"));
		return EMPTY_MESSAGE;
	}
